...
TimeLineHistoryEntry *timeline_history;
int timeline_num_entries;
				   ...
				   XLogRecPtr *lastchkptredo)
{
	...
	for (;;)
	{
		...
		{
			if (errormsg)
				elog(ERROR, "could not find previous WAL record at %X/%X: %s\n",
						 (uint32) (searchptr >> 32), (uint32) (searchptr),
						 errormsg);
			else
				elog(ERROR, "could not find previous WAL record at %X/%X\n",
						 (uint32) (searchptr >> 32), (uint32) (searchptr));
		}
				   ...
				   TimeLineID *pageTLI)
{
	...
	if (xlogreadfd < 0)
	{
		 ...
		 * accordingly.
		 */
		while (private->tliIndex < timeline_num_entries - 1 &&
				timeline_history[private->tliIndex].end < targetSegEnd)
			private->tliIndex++;
		while (private->tliIndex > 0 &&
				timeline_history[private->tliIndex].begin >= targetSegEnd)
			private->tliIndex--;
	...
	*pageTLI = timeline_history[private->tliIndex].tli;
	return XLOG_BLCKSZ;
}
